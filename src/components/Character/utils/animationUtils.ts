...
const setAnimations = (gltf: GLTF) => {
  ...
  if (gltf.animations) {
    ...
    if (typingAction) {
      // The typing action is NOT auto-run here — it used to play at
      // weight 1 permanently, which is why the character looked like
      // it was robotically typing the entire page. The scene
      // controller (sceneState.ts) now owns this action's weight and
      // plays it in short bursts tied to the active section.
      typingAction.enabled = true;
      typingAction.setEffectiveWeight(0);
      typingAction.play();
  ...
  return { mixer, startIntro, hover, typingAction };
};
